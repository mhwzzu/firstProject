...
public class RecommendationController {
    ...
    @GetMapping("/today") public RecommendationResponse today(@RequestParam(required=false)String city,@RequestParam(required=false)String prompt,@RequestParam(required=false)String tags,@RequestParam(required=false)Double latitude,@RequestParam(required=false)Double longitude,@RequestParam(required=false)Integer budget,@RequestParam(required=false)Integer travelMinutes,HttpServletRequest request){Long user=sessions.requireUserId(request);CoupleSpace space=spaces.requireCurrentSpace(user);return recommendations.today(space.getId(),new DecisionQuery(city,prompt,tags,latitude,longitude,budget,travelMinutes));}
    @GetMapping("/weekend") public RecommendationResponse weekend(@RequestParam(required=false)String city,@RequestParam(required=false)String prompt,@RequestParam(required=false)String tags,@RequestParam(required=false)Double latitude,@RequestParam(required=false)Double longitude,@RequestParam(required=false)Integer budget,@RequestParam(required=false)Integer travelMinutes,HttpServletRequest request){Long user=sessions.requireUserId(request);CoupleSpace space=spaces.requireCurrentSpace(user);return recommendations.weekend(space.getId(),new DecisionQuery(city,prompt,tags,latitude,longitude,budget,travelMinutes));}
}
